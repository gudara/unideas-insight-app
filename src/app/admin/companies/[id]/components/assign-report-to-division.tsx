...
import { updateReports } from "./assign-report-to-division-action";

interface AssignReportToDivisionProps {
    division: Division;
    selectedIdList: number[];
}

export const AssignReportToDivision: React.FC<AssignReportToDivisionProps> = ({ division, selectedIdList }) => {
    ...
    function onSelectTrigger(isSelected: boolean, item: Report): void {
        startStatusUpdate(async () => {
            const da = await updateReports(division.id, isSelected, [item]);
            if (da.data) {
        ...
        <>
            {
                !!division &&
                <Drawer onClose={() => {
                    queryClient.refetchQueries({ queryKey: ['division-all-reports', division] })
                }}>
